function code = Level3b_Exp_AK_171108_2Cond
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/2 &&...
    ...
    if vr.position(1) < 0 && ismember(vr.currentCueWorld,[5 6]) % left turn, worlds 5&6
        ...
        elseif vr.currentCueWorld == 2 || vr.currentCueWorld == 6 % give mult awards for no tower?
            if vr.mulRewards == 0
                vr = giveReward_AK(vr,1);
            elseif vr.mulRewards == 1
                vr = giveReward_AK(vr,2);
    ...
    elseif  vr.position(1) > 0 && ismember(vr.currentCueWorld,[3 4]) % right turn, worlds 3&4
        ...
        if vr.currentCueWorld == 3 || vr.currentCueWorld == 7
            ...
            vr.whichWorldFlag = vr.currentCueWorld + 1;
            vr.numRewardsTower = vr.numRewardsTower + 1;
        elseif vr.currentCueWorld == 4 || vr.currentCueWorld == 8 % give mult awards for no tower?
            if vr.mulRewards == 0
                vr = giveReward_AK(vr,1);
            elseif vr.mulRewards == 1
                vr = giveReward_AK(vr,2);
        ...
        vr.trialResults(1,size(vr.trialResults,2)+1) = 1;
        vr.streak = vr.streak + 1;
    else % otherwise it's a wrong turn
        vr.isReward = 0;
...
if vr.inITI == 1 % iti behavior
    vr.itiTime = toc(vr.itiStartTime);
    
    if vr.cellWrite
        % Summary: Success | Trial Time
        % One cell per trial
        dataStruct=struct('success',vr.isReward,'conds',vr.currentCueWorld,...
            'Tower',ismember(vr.currentCueWorld,[3,5]));
    ...
    if vr.itiTime > vr.itiDur % end iti
        vr.inITI = 0;    
        vr.cellWrite = true; % write data to cell flag
        % adapt to adjust for biases?
