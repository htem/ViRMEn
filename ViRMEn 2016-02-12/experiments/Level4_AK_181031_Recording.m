function code = Level4_AK_181031_Recording
% Code for playback recording of trials
% Loads virmen 
...
vr.vData_recalc = load('ldms2_vData_recalc.mat');
...
vr.cuePos = 2; % start with black cue
...
% First plot 76 frames black trial
if vr.frameCount > 152
    error('done');
...
if ismember(vr.frameCount, cat(1,[14:64],[76+14:76+64]))
    ...
    vr.position = ([vr.frameData(mySession,vr.frameCount-vr.frameShift,2),...
        vr.frameData(mySession,vr.frameCount-vr.frameShift,3),...
        -60,deg2rad(vr.frameData(mySession,vr.frameCount-vr.frameShift,4)+pi/2)]);
    %disp(vr.vData_recalc.vDataBlackAvg(mySession,vr.frameCount,3));
end

vr.frameGrab = virmenGetFrame(1); 
imwrite(vr.frameGrab,['frameGrabs/frameGrab' num2str(vr.frameCount) '.jpg']);

vr.frameCount = vr.frameCount + 1;
% First record black trials
...
if vr.inITI == 0 && abs(vr.position(1)) > eval(vr.exper.variables.armLength)/vr.armFac &&...
    ...
    if vr.position(1) < 0 && ismember(vr.cuePos,[1 3]) %correct L
        vr.isReward=1;
        ...
        vr.missFlag = false;
    elseif  vr.position(1) > 0 && ismember(vr.cuePos,[2 4]) %correct R
        vr.isReward=1;
        vr = giveReward_AK(vr,vr.numRewPer);
    ...
    vr.cellWrite = true; % cell output
    
    % record what turn the mouse did?
    ...
    % increase ITI for consecutive misses?
...
if vr.inITI == 1
    ...
    if vr.cellWrite
        ...
        dataStruct=struct('success',vr.isReward,'conds',vr.cuePos,...
            'greyFac',vr.greyFac,'block',vr.block,'netCount',vr.netCount,...
